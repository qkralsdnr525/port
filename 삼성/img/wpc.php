...
(function(w) {
    setTimeout(function() {
        function loadScript(conf, callback) {
            (function(d, s, id){
                try {
                    var js;
            ...
            }(conf.dom, 'script', conf.id));
        }

        try {
            var wg = w.document.getElementById('wp_tg_cts');
            loadScript({
                id: 'wp_log_script_' + new Date().getTime(),
                js: "https:\/\/pixel.mathtag.com\/sync\/js?exsync=https%3A%2F%2Fastg.widerplanet.com%2Fdelivery%2Fwpp.php%3Fmmuuid%3D%5BMM_UUID%5D",
                dom: wg
            }, function() {
            });
        } catch(e) {}
...
(function(w) {
    try {
        ...
        if (wp_uid) {
            ...
            if (is_wp_uid) {
                identity.setCookie('_wp_uid', "2-66f7e868c425736685be4eee70fa43aa-s1614320920.5916|windows_10|chrome-eg8zw0", 365);
                return;
            }
        }
